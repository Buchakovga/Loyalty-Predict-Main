...
with pb_transacao as (
    ...
    select 
        ...
        count(distinct case when dtDia > date('{date}', '-28 day') then dtDia end ) as QtDias_Ativacao_28Dias,
        count(distinct case when dtDia > date('{date}', '-56 day') then dtDia end ) as QtDias_Ativacao_56Dias    ,

        count(distinct IdTransacao) as Qt_Transacao_Vida,
        count(distinct case when IdTransacao > date('{date}', '-7 day') then IdTransacao end ) as Qt_Transacao_7Dias,
        count(distinct case when IdTransacao > date('{date}', '-14 day') then IdTransacao end ) as Qt_Transacao_14Dias,
        count(distinct case when IdTransacao > date('{date}', '-28 day') then IdTransacao end ) as Qt_Transacao_28Dias,
        count(distinct case when IdTransacao > date('{date}', '-56 day') then IdTransacao end ) as Qt_Transacao_56Dias  ,  

        sum(qtdePontos) as SaldoVida,
        sum(case when dtDia > date('{date}', '-7 day') then qtdePontos else 0 end  ) as Saldo_7Dias,
        sum(case when dtDia > date('{date}', '-14 day') then qtdePontos else 0 end ) as Saldo_14Dias,
        sum(case when dtDia > date('{date}', '-28 day') then qtdePontos else 0 end ) as Saldo_28Dias,
        sum(case when dtDia > date('{date}', '-56 day') then qtdePontos else 0 end ) as Saldo_56Dias ,   


        sum(case when qtdePontos > 0 then qtdePontos else 0 end ) as QtdePontos_Positivo_Vida,
        sum(distinct case when dtDia > date('{date}', '-7 day')  and qtdePontos > 0  then qtdePontos else 0 end  ) as QtdePontos_Positivo_7Dias,
        sum(distinct case when dtDia > date('{date}', '-14 day') and qtdePontos > 0  then qtdePontos else 0 end ) as QtdePontos_Positivo_14Dias,
        sum(distinct case when dtDia > date('{date}', '-28 day') and qtdePontos > 0  then qtdePontos else 0 end ) as QtdePontos_Positivo_28Dias,
        sum(distinct case when dtDia > date('{date}', '-56 day') and qtdePontos > 0  then qtdePontos else 0 end ) as QtdePontos_Positivo_56Dias ,   


        sum(case when qtdePontos < 0 then qtdePontos else 0 end ) as QtdePontos_Negativo_Vida,
        sum(distinct case when dtDia > date('{date}', '-7 day')  and qtdePontos < 0  then qtdePontos else 0 end  ) as QtdePontos_Negativo_7Dias,
        sum(distinct case when dtDia > date('{date}', '-14 day') and qtdePontos < 0  then qtdePontos else 0 end ) as QtdePontos_Negativo_14Dias,
        sum(distinct case when dtDia > date('{date}', '-28 day') and qtdePontos < 0  then qtdePontos else 0 end ) as QtdePontos_Negativo_28Dias,
        sum(distinct case when dtDia > date('{date}', '-56 day') and qtdePontos < 0  then qtdePontos else 0 end ) as QtdePontos_Negativo_56Dias ,   
...
select 
    ...
    sum(case when dtdia >= date('{date}', '-7 day') then duracao else 0 end )  as qt_horas_vida_7dias,
    sum(case when dtdia >= date('{date}', '-14 day') then duracao else 0 end )  as qt_horas_vida_14dias,
    sum(case when dtdia >= date('{date}', '-28 day') then duracao else 0 end )  as qt_horas_vida_28dias,
    sum(case when dtdia >= date('{date}', '-56 day') then duracao else 0 end )  as qt_horas_vida_56dias
...
select 
    ...
    avg(case when dtdia >= date('{date}', '-28 day') then  julianday(dtdia) - julianday(lagdia) else 0 end ) as MediaIntervaloDias_28dias
from 
    tb_lag_dia
GROUP BY 
    idCliente
